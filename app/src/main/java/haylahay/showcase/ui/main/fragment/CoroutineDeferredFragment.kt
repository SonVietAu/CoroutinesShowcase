...
class CoroutineDeferredFragment : CSBaseFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        headerTV.text = "Deferred"

        lottoNumberSetsRV.layoutManager = LinearLayoutManager(this.context)

        nextBtn.text = "Next With Deferred"
        nextBtn.setOnClickListener {
            viewModel.getNextWithDeferred()
        }

        refreshCheckedBtn.visibility = View.GONE

        refreshAllBtn.setOnClickListener {
            viewModel.refreshAllLottoNumbersWithDeferredAndCollection()
        }
    ...
    override fun onResume() {
        super.onResume()
        informationTV.text = activity?.getText(R.string.deferred_information)
    }
